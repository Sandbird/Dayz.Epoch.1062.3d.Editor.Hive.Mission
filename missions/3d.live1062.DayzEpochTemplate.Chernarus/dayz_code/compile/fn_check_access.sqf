...
_playerUID = _player getVariable ["playerUID", 0]; //getPlayerUID _player;
_characterID = player getVariable ["CharacterID","0"];
if (DZE_permanentPlot) then {
	_targetOwnerUID = if (isPlayer _target) then {_target getVariable ["playerUID", 0]} else { _target getVariable ["ownerPUID","0"] };
