...
if(SheildMe && zombieShield) then {
	...
	if(EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has enabled zombie shield for distance: %3",name _player,(_player getVariable["playerUID", 0]),ZombieDistance];
		publicVariableServer "EAT_PVEH_usageLogger";
	};
} else {
	// Tool use logger
	if(EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has disabled zombie shield",name _player,(_player getVariable["playerUID", 0])];
		publicVariableServer "EAT_PVEH_usageLogger";
	};
};
