...
if (EAT_flying2) then 
{
	...
	if(EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has added flying",name player,(player getVariable["playerUID", 0])];
	...
	// Tool use logger
	if(EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has DISABLED flying",name player,(player getVariable["playerUID", 0])];
		publicVariableServer "EAT_PVEH_usageLogger";
	};
