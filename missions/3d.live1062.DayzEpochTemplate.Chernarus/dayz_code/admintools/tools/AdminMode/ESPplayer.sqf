...
if(playerESP2) then {
	...
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has player ESP",name player,(player getVariable["playerUID", 0])];
...
if(!playerESP2) then {
	// Tool use logger
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has DISABLED player ESP",name player,(player getVariable["playerUID", 0])];
		publicVariableServer "EAT_PVEH_usageLogger";
	};
};
