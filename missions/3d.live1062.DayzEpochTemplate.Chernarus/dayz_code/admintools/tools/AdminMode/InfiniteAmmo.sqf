...
if(EAT_infAmmo2) then {
	...
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has turned ON infinite ammo",name player,(player getVariable["playerUID", 0])];
...
// Tool use logger
if(EAT_logMajorTool) then {
	EAT_PVEH_usageLogger = format["%1 %2 -- has turned OFF infinite ammo",name player,(player getVariable["playerUID", 0])];
	publicVariableServer "EAT_PVEH_usageLogger";
};
