...
if (_obj isKindOf "LandVehicle" || _obj isKindOf "Air" || _obj isKindOf "Ship") then {
	...
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has unlocked vehicle: %3 with ID:%4",name _player,(_player getVariable["playerUID", 0]),_obj,_objectID];
	...
	if(_objType in DZE_LockedStorage) then {
		...
		PVDZE_handleSafeGear call server_handleSafeGear;
		...
		// Tool use logger
		if(EAT_logMajorTool) then {
			EAT_PVEH_usageLogger = format["%1 %2 -- has unlocked a safe - ID:%3 UID:%4",name _player,(_player getVariable["playerUID", 0]),_objectID,_ownerID];
			publicVariableServer "EAT_PVEH_usageLogger";
		};		
	} else {
		...
		// Tool use logger
		if(EAT_logMajorTool) then {
			EAT_PVEH_usageLogger = format["%1 %2 -- has unlocked a door - ID:%3 Combo:%4",name _player,(_player getVariable["playerUID", 0]),_objectID,_objectCharacterID];
			publicVariableServer "EAT_PVEH_usageLogger";
		};
	};
};
