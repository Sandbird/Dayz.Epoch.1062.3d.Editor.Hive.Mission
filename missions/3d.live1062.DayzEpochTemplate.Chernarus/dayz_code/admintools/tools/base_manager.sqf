...
fn_BCExport = {
	...
	if(EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has exported a base",name player,(player getVariable["playerUID", 0])];
...
fn_BCPaste = {
	...
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has pasted a copied base",name player,(player getVariable["playerUID", 0])];
		publicVariableServer "EAT_PVEH_usageLogger";
	};	
};
...
fn_BCConfirmDelete = {
	...
	// Tool use logger
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has deleted %3 items using deletebase",name player,(player getVariable["playerUID", 0]),count _objects];
...
fn_BCSaveToDb = {
	...
	// Tool use logger
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has placed a saved base",name player,(player getVariable["playerUID", 0])];
		publicVariableServer "EAT_PVEH_usageLogger";
	};
};
