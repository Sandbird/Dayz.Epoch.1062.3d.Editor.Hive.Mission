...
if((EAT_ActionMenuPlayers && !EAT_isAdmin && EAT_ActionMenuHotkey) || (EAT_ActionMenuAdmins && EAT_isAdmin && EAT_ActionMenuHotkey)) then {
	...
	Tab_KEY = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 15) then {[] execVM ""dayz_code\admintools\actionMenu\ActionsMenu.sqf"";true};"];
} else {
	if((EAT_ActionMenuPlayers && !EAT_isAdmin) || (EAT_ActionMenuAdmins && EAT_isAdmin)) then {
		[] spawn {
			...
			while {alive player} do {
				if(_idx == -1) then {
					_idx = (vehicle player) addaction [("<t color=""#FE9A2E"">" + ("Action Menu") + "</t>"),"dayz_code\admintools\actionMenu\ActionsMenu.sqf","",-107,false,true,"",""];
					_veh = vehicle player;
				};
...
if (EAT_isAdmin && EAT_AdminMenuHotkey) then {
	...
	F2_KEY = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 60) then {[] execVM ""dayz_code\admintools\AdminToolsMain.sqf"";true};"];
	// Load key macros
	[]execVM "dayz_code\admintools\KeyBindings\FunctionKeys.sqf";
	[]execVM "dayz_code\admintools\KeyBindings\NumberKeys.sqf";
	
	// Tool use logger
	if(EAT_logMajorTool || EAT_logMinorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has logged on",name player,(player getVariable["playerUID", 0])];
	...
	if (EAT_isAdmin) then {
		[] spawn {
			...
			{
				if (_idx == -1) then {
					_idx = (vehicle player) addaction [("<t color=""#585858"">" + ("Admin Menu") +"</t>"),"dayz_code\admintools\AdminToolsMain.sqf","",7,false,true,"",""];
					_veh = vehicle player;
				};
				if (_veh != vehicle player) then {
					_veh removeAction _idx;
...
if (EAT_DebugMonitor && !isDedicated) then {[] execVM "dayz_code\admintools\debug_monitor\debug_monitor.sqf";
