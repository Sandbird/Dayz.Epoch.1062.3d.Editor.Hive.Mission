...
_checkBuildingCollision =
{
    ...
    local _result = false;
    {
        if (_x != _object && { _x distance _object < 1.5 || { _x getVariable ["ownerArray", [""]] select 0 != (player getVariable ["playerUID", 0]) } } ) exitWith
	...
	//Trace paths
    {
        ...
        for "_i" from 1 to count _x - 1 do
        {
            ...
            {			 
                if (!_wall || { !(_x isKindOf "DZ_buildables" && { _x getVariable ["ownerArray", [""]] select 0 == (player getVariable ["playerUID", 0]) }) }) then
                {
...
_actionCancel = player addAction [localize "str_player_build_cancel", "dayz_code\actions\object_build.sqf", [_object, _requiredParts, _classname, _text, false, 0, "none"], 1, true, true, "", "0 != count Dayz_constructionContext"];
...
while {Dayz_constructionContext select 4} do {
	...
	if (isNull _objColliding and _maxplanting <= _emergingLevel and !(call _checkOnRoad)) then { 
		...
		if (_actionBuildHidden) then {
			...
			_actionBuild = player addAction [localize "str_player_build_complete", "dayz_code\actions\object_build.sqf", [_object, _requiredParts , _classname, _text, true, 20, _sfx], 1, true, true, "", "0 != count Dayz_constructionContext"];
			_actionCancel = player addAction [localize "str_player_build_cancel", "dayz_code\actions\object_build.sqf", [_object, _requiredParts  , _classname, _text, false, 0, "none"], 1, true, true, "", "0 != count Dayz_constructionContext"];
	   };
	} else {
		if (!_actionBuildHidden) then {
