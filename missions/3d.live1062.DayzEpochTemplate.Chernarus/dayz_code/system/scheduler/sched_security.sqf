...
sched_security = {
	...
	if (typeName player != "OBJECT") then {
		PVDZ_sec_atp = format["WARNING typename error for play3r UID#%1", player getVariable ["playerUID", 0]];
	...
	switch (true) do {
		case (1==0) : {
			PVDZ_sec_atp = format["WARNING true error for play3r UID#%1", player getVariable ["playerUID", 0]];
			publicVariable "PVDZ_sec_atp";
			endMission "LOSER";
		};
	};
	...
	if (_time - _otime > 1 or _time < _otime) then {
		...
		if (_timeTrickCount > 2) then {
			PVDZ_sec_atp = format["WARNING unusual time shift for UID#%1", player getVariable ["playerUID", 0]];
			publicVariable "PVDZ_sec_atp";
		};
	};
	...
	if (_pid != _netid) then {
		...
		if (_idTrickCount > 2) then {
			PVDZ_sec_atp = format["WARNING unusual outfit swap for UID#%1", player getVariable ["playerUID", 0]];
			publicVariable "PVDZ_sec_atp";
		};
	};
	...
	if (!isNull _grp) then {
		...
		if (count _list > 1) then {
			...
			for "_i" from 2 to (count _list)-1 do {
			    ...
			    if (!isNull _u) then {
			        _pos = getPosATL _u;
			        PVDZ_sec_atp = format ["From play3r UID#%1: illegal gr0up m3mber %2 at %3,%4... Server may be compromised!", player getVariable ["playerUID", 0], typeOf _u, _pos select 0, _pos select 1];
			        publicVariable "PVDZ_sec_atp";
			        deleteVehicle _u;
			        _quit = true;
			    };
