...
if (toLower worldName == "chernarus") then { //need to add building coordinates for other maps
	{
		...
		_fckingcode = {
			private ["_building","_part","_dmgLvl","_who","_ammo","_dist","_playerUID"];
			_building = _this select 0;
			_part = _this select 1;
			_dmgLvl = 1 min (_this select 2);
			_who = _this select 3;
			_playerUID = _who getVariable ["playerUID", 0];
				...
				if (isServer) then {
					...
					if (isNull _who) then {
						if (_ammo != "" && _ammo isKindOf "HelicopterExploSmall") then {
							...
							PVDZ_sec_atp = format ["UID#%1 d4maged %2 %5 to %3pct with ammo %4 at dist4nce %6m.",_playerUID, typeOf _building, round (100 * _dmgLvl), _ammo, _part, _dist];
							publicVariable "PVDZ_sec_atp";
						};
					} else {
						if (_who == player) then {
							_dist = round (_who distance _building);
...
if (isServer) then {
	...
	"PVDZ_plr_Login2"		addPublicVariableEventHandler {_id = (_this select 1) call server_playerSetup};
	//};
	...
	//"PVDZ_dayzCarBomb" addPublicVariableEventHandler {[_this select 1] execVM "dayz_code\actions\detonate_bomb.sqf";};
	...
	"PVDZ_Server_processSetAccessCode" addPublicVariableEventHandler {
		...
		if ((_ownerArray select 0) == (_unitSending getVariable ["playerUID", "0"])) then {
...
if (!isDedicated) then {
	...
	if (dayz_enableFlies) then {
		...
		call compile preprocessFileLineNumbers "dayz_code\compile\client_flies.sqf";
	};
