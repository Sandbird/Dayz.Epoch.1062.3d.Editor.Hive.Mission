		...
		_UID = _x select 2;
			if (_ID == "0") then {
				if (_UID != "0") then {
					...
					_key = format["UPDATE `Object_DATA` SET `Datestamp` = CURRENT_TIMESTAMP, `Damage` = '0' WHERE `ObjectUID` = '%1'", _UID]; // use UID if not "0" and ID is "0"
					_key call server_hiveWrite;
				};
			} else {
				_obj setDamage 0;
				//_key = format["CHILD:396:%1:", _ID];
				_key = format["UPDATE `Object_DATA` SET `Datestamp` = CURRENT_TIMESTAMP, `Damage` = '0' WHERE `ObjectID` = '%1'", _ID]; // use UID if not "0" and ID is "0"
				_key call server_hiveWrite;
			};
	} count _objectsInfo;
