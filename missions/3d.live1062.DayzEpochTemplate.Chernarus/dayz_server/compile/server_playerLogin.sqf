...
diag_log("Server as player in PlayerLogin is:"+_playerName);
...
while {_doLoop < 5} do {
	...
	_key = format["SELECT c.CharacterID, c.Inventory, c.Backpack, TIMESTAMPDIFF(MINUTE,c.Datestamp,c.LastLogin) as SurvivalTime, TIMESTAMPDIFF(MINUTE,c.LastAte,NOW()) as MinsLastAte, TIMESTAMPDIFF(MINUTE,c.LastDrank,NOW()) as MinsLastDrank, c.Model, c.Coins, p.playerGroup, p.PlayerCoins, p.BankCoins FROM Character_DATA c LEFT JOIN Player_DATA p ON p.PlayerUID = c.PlayerUID WHERE c.PlayerUID = %1 AND c.Alive = 1 ORDER BY c.CharacterID DESC LIMIT 1",_playerID];  //_playerID,dayZ_instance,_playerName
...
//diag_log format[">>>>>>>>> Got the serverLogin data: %1",_primary];
...
//Process request
_newPlayer = 	false; //_newPlayer = _primary select 1;
...
if (!_isNew) then {
	...
	_group = _primary select 8;
	_playerCoins = _primary select 9;
	_BankCoins = _primary select 10;
	_hiveVer = 0.96; //_hiveVer = _primary select 12;
	if !(_model in AllPlayers) then {_model = "Survivor2_DZ";};
} else {
	/*
	// Always load from the db a character...no new function
	//
	diag_log format ["extDB3: _primary: %1", _primary];
	_isInfected = if (DZE_PlayerZed) then {1} else {0};
	_model = _primary select 4;
	_group = _primary select 5;
	_playerCoins = _primary select 6;
	_BankCoins = _primary select 7;
	_hiveVer = 0.96; //_hiveVer = _primary select 8;
	...
	if (_isInfected != 1) then {
		...
		_key = format["UPDATE `Character_DATA` SET `Inventory` = '%2' , `Backpack` = '%3' WHERE `CharacterID` = '%1'",_charID,[_wpns,_mags],[_bcpk,[],[]]];
