...
{
	if ((_x getVariable["playerUID", 0]) == _playerUID) exitWith { _playerObj = _x; _playerPos = getPosATL _playerObj;};
} count playableUnits;
...
//diag_log format["get: %1 (%2), sent: %3 (%4)",typeName ((_playerObj getVariable["playerUID", 0])), getPlayerUID _playerObj, typeName _playerUID, _playerUID];
