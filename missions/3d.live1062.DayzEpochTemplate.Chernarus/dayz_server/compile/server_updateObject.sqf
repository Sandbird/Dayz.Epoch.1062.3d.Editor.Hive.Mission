...
_objectID = format["%1", _objectID];
_objectUID = format["%1", _objectUID];
...
if (!(_class in DZE_safeVehicle) && !locked _object) then {
	...
	if ((isNil {_objectID}) && (isNil {_objectUID}) && (vehicle _object getVariable ["DZAI",0] != 1)) then {
...
_object_damage = {
	...
	if (_forced) then {        
		...
		if (diag_ticktime - _lastUpdate > 10) then {
			if !(_object in needUpdate_objects) then {
				//diag_log format["DEBUG: Monitoring: %1 lastupdate: %2",_object, (diag_ticktime - _lastUpdate)];
	...
	if (_recorddmg) then {
		if (_objectID == "0") then {
			...
			_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectUID = '%1'",_objectUID,_array,_damage];
		} else {
...
_objWallDamage = {
	...
	if (_objectID == "0") then {
		...
		_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectUID = '%1'",_objectUID,[],_damage];
	} else {
		//_key = format["CHILD:306:%1:%2:%3:",_objectID,[],_damage];
		_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectID = '%1'",_objectID,[],_damage];
	};
	#ifdef OBJECT_DEBUG
	diag_log ("HIVE: WRITE: "+ str(_key));
	#endif
...
_object_killed = {
	...
	if (_objectID == "0") then {
		...
		_key = format["DELETE FROM Object_DATA WHERE `ObjectUID` = '%1'",_objectUID];
	} else {
		//_key = format["CHILD:306:%1:%2:%3:",_objectID,[],1];
		_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectID = '%1'",_objectID,[],1];
	};
	_key call server_hiveWrite;
...
_object_maintenance = {
	...
	if (_objectID == "0") then {
		...
		_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectUID = '%1'",_objectUID,[],0];
	} else {
		//_key = format["CHILD:303:%1:%2:",_objectID,_ownerArray];
		//_key = format["CHILD:306:%1:%2:%3:",_objectID,[],0];
		_key = format["UPDATE OBJECT_data SET Hitpoints = '%2', Damage = '%3' WHERE ObjectID = '%1'",_objectID,[],0];
	};
