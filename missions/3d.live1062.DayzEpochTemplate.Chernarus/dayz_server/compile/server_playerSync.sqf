...
_exitReason = switch true do {
	case (isNil "_characterID"): {("ERROR: Cannot Sync Character " + _name + " has nil characterID "+_characterID)}; //Unit is null
	...
	case (_characterID == "0"): {("ERROR: Cannot Sync Character " + _name + " has no characterID "+_characterID)};
...
if (Z_SingleCurrency) then {
	...
	//_key call server_hiveWrite;
	//sleep 0.5;
	_key = format["UPDATE `character_data` SET `Inventory` = '%1',`Backpack` = '%2',`Worldspace` = '%3',`Medical` = '%4',`CurrentState` = '%5',`HeadshotsZ` = '%6',`KillsZ` = '%7',`KillsH` = '%8',`KillsB` = '%9',`Humanity` = '%10', `DistanceFoot` = '%11', `Model` = '%12', `Duration` = '%13', `LastLogin` = CURRENT_TIMESTAMP WHERE `character_data`.`CharacterID` = '%14'",_playerGear, _playerBackp, _playerPos, _medical, _currentState,_headShots, _kills, _killsH,_killsB, _humanity, _distanceFoot, _currentModel, _timeSince, _characterID];
};
...
if (Z_SingleCurrency) then { //update global coins
	...
	if (_globalCoins >= 0 && _bankCoins >= 0) then {
		_key = format["UPDATE `Player_DATA` SET `PlayerCoins`= '%2', `BankCoins`= '%3' WHERE `PlayerUID` = '%1'", _playerUID, _globalCoins, _bankCoins];
		_key call server_hiveWrite;
	};
	if (_globalCoins >= 0) then {
		_key = format["UPDATE `Player_DATA` SET `PlayerCoins`= '%2' WHERE `PlayerUID` = '%1'", _playerUID, _globalCoins];
		_key call server_hiveWrite;
	};
	if (_bankCoins >= 0) then {
		_key = format["UPDATE `Player_DATA` SET `BankCoins`= '%2' WHERE `PlayerUID` = '%1'", _playerUID, _bankCoins];
		_key call server_hiveWrite;
	};
...
//	diag_log format["++++++++++PLAYER SYNC+++++++++++ _character:%1, _characterID:%2, _Magazines:%3, _playerUID:%4", _character, _characterID, _magazines, _playerUID];
};
