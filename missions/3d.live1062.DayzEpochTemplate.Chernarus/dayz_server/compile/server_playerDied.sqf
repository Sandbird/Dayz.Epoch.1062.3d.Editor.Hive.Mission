...
if (_characterID != "0") then {
	...
	_key = format["UPDATE `Character_DATA` SET `Alive` = 0, `Infected` = '%3', `LastLogin` = DATE_SUB(CURRENT_TIMESTAMP, INTERVAL %2 MINUTE) WHERE `CharacterID` = '%1' AND `Alive` = 1", _characterID,_minutes,_infected]; // use UID if not "0" and ID is "0"
...
if (_playerName != "unknown" or _sourceName != "unknown") then {
	if (toLower DZE_DeathMsgChat != "none" or DZE_DeathMsgRolling or DZE_DeathMsgDynamicText) then {
		...
		{
			if !(_x getVariable ["playerUID", 0] in ["",_playerID]) then {
