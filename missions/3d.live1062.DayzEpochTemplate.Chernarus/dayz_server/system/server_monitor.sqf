private ["_date","_year","_month","_day","_hour","_minute","_date1","_key","_objectCount","_dir","_point","_i","_action","_dam","_selection","_wantExplosiveParts","_entity","_worldspace","_damage","_booleans","_rawData","_ObjectID","_class","_CharacterID","_inventory","_hitpoints","_fuel","_id","_objectArray","_script","_result","_outcome","_shutdown","_res","_totsVehicles"];
[] execVM "dayz_server\system\s_fps.sqf"; //server monitor FPS (writes each ~181s diag_fps+181s diag_fpsmin*)
...
if (DefaultTruePreMadeFalse) then {
	...
	_result = _key call server_hiveReadWrite;
	_objectCount = (_result select 0 select 0);
	diag_log ("HIVE: found "+str(_objectCount)+" objects" );
	diag_log ("HIVE: Commence Object Streaming...");
	for "_i" from 0 to (_objectCount - 1) do {
		private ["_hiveResponse","_key","_response"];
		 //_key = [2,"VGAR",(format ["server_getObjects:%1%2",dayZ_instance, _i])] call EXTDB_fnc_APICall;
		...
		//diag_log format["::::::: 3- Final obj ::::: %1",_response];
		//_response = call compile _response;
		//sleep .001;
		//diag_log (format["HIVE dbg %1 %2", typeName _response select 2, _response select 2]);
...

{
	...
	//Parse Array
	//diag_log (format["HIVE OBJ to spawn %1", _x]);
	//[2,3,"UAZ_CDF",0,[94,[6835.47,2439.34,9.53707]],[],[["glass1",0.111],["glass2",0.558],["glass3",0.628],["glass4",0.229],["wheel_1_1_steering",0.695],["wheel_1_2_steering",0.508],["wheel_2_1_steering",1],["wheel_2_2_steering",0.807],["motor",0.042],["sklo predni P",0.848],["sklo predni L",0.48],["karoserie",0.97],["palivo",0.34],["wheel_1_4_steering",0.726],["wheel_2_4_steering",0.54],["wheel_1_3_steering",0.291],["wheel_2_3_steering",0.382]],0.48081,0.00923,0]
	_idKey = 		str (_x select 0);    //format ["%1",[_x select 0]];
	_vUID = 		format["%1",(_x select 1)];
	_type =			_x select 2;
	_ownerID = 		str (_x select 3);
	...
	*/
		...
		//diag_log("TTTTTTTTTTTTYYYYYPE"+_type);
		...
		if (_object isKindOf "AllVehicles") then {
			...
			if (!_isSafeObject) then {
				...
				if(_ownerID != "0" and !(_object isKindOf "Bicycle")) then {
					...
					//diag_log format["========================= serviceVehCounter: %1, type: %2", (count serverVehicleCounter), _type];
...
if (_hiveLoaded) then {
	diag_log format[":::HIVE LOADED:::"];
	systemChat format [":::HIVE LOADED:::"];
		//  spawn_vehicles
		// Get all buildings and roads only once. Very taxing, but only on first startup
		_startTime = diag_tickTime;
		{
			if (isClass (missionConfigFile >> "CfgLoot" >> "Buildings" >> (typeOf _x))) then
			{
				BuildingList set [count BuildingList,_x];
			};
		} forEach (MarkerPosition nearObjects ["building",(MarkerPSize select 1)]);
		//diag_log format["RoadList::::%1", RoadList];
		RoadList = MarkerPosition nearRoads (dayz_MapArea/2);
		...
		{[_x,"gear"] call server_updateObject} count vehiclesToUpdate;
};
...
// # END OF STREAMING #
if (dayz_townGenerator) then {
	call compile preprocessFileLineNumbers "dayz_server\compile\server_plantSpawner.sqf"; // Draw the pseudo random seeds
};
#ifndef OBJECT_DEBUG
	object_debug = false;
#else
...
[] execVM "dayz_server\compile\server_spawnCarePackages.sqf";
[] execVM "dayz_server\compile\server_spawnCrashSites.sqf";
...
"PVDZ_objgather_Knockdown" addPublicVariableEventHandler {
	...
	_uid = _player getVariable ["playerUID", 0]; //getPlayerUID _player; 
		...
		if (!isNil "_traderData") then {
			{
				...
				_result = _key call server_hiveReadWrite;
				_data = (_result select 0 select 0);		
